  ...
  <div class="box">
    ...
    <el-dialog :visible.sync="dialogFrom" title="新增教学场景" width="600px">
      <div class="modal-body">
        ...
        <el-form :model="secdfrom" ref="secdfrom" :rules="hardwarerules" v-show="flagView === 2">
          <!-- 硬件配置 -->
          ...
          <el-form-item label="USB端口" label-width="100px">
            ...
            <el-popover placement="bottom" title="" width="250" trigger="hover" content="1.使用USB 3.0重定向，则在桌面内采用USB 3.0的控制器；2.使用USB 2.0重定向，则在桌面内采用USB 2.0的控制器；3.无论启用何种方式，任意协议类型的USB设备可以接入终端任意协议类型的接口上重定向使用；4.KVM虚拟化类型下，Windows 7及以上操作系统推荐使用USB 3.0重定向，Windows XP系统只能使用USB 2.0重定向。">
              <li class="tip" slot="reference">?</li>
            </el-popover>
            <el-checkbox v-model="show_desk_info" class="el-show_desk_info">屏幕水印</el-checkbox>
            <el-popover placement="bottom" title="" width="250" trigger="hover" content="1. 使用加速（不安全）选项，是指在保障磁盘一定读速度的前提下，实现磁盘写加速。但当遇到服务器异常关机或重启时，会有一定机率造成数据丢失。">
              <li class="tip" slot="reference">?</li>
            </el-popover>
          </el-form-item>
          <!-- 磁盘缓存 -->
          <el-form-item label="磁盘缓存" label-width="100px">
            ...
            <el-popover placement="bottom" title="" width="250" trigger="hover" content="1、启用半虚拟化声卡后， 桌面内视频播放更加流畅，声音效果更佳；</br>2、启用后，桌面无法执行挂起、迁移操作。">
              <li class="tip" slot="reference">?</li>
            </el-popover>
        ...
        <el-form :rules="hostrules" v-show="flagView === 3" v-model="hostfrom" ref="hostfrom">
          ...
          <el-form-item label="计算节点" label-width="100px" required>
            ...
            <div class="checkbnt">
              ...
              <button type="button" class="btn btn-sm btn-default firstbtn text-left"
                :disabled="checkboxGroup.length === 0 && hostNum === 0"
                :class="{ 'btn-choice': checkboxGroup.length !== 0 || hostNum !== 0 }" @click="twoDialog">
                高级
              </button>
            </div>
          </el-form-item>
          <!-- 桌面数 -->
          <el-form-item label="桌面数" label-width="100px" prop="hostNum" required>
            <el-input-number v-model="hostfrom.hostNum" controls-position="right" :min="0" :max="30"
              :disabled="checkboxGroup.length === 0" class="el-hostNum" v-show="hostType === '0'">
            </el-input-number>
    ...
    <el-dialog :visible.sync="dialogtable" title="高级" width="600px" class="dialogtable">
      <el-form v-model="hostListFrom" :rules="hostStorage" ref="hostListFrom">
        ...
        <el-form-item label="系统盘镜像存储设备" label-width="160px">
          <el-select v-model="hostListFrom.image_storage_performance" class="el-resourceId">
        ...
        <el-form-item label="数据盘镜像存储设备" label-width="160px">
          <el-select v-model="hostListFrom.image_storage_capacity" class="el-resourceId">
        ...
        <el-form-item label="系统盘存储设备" label-width="160px">
          <el-select v-model="hostListFrom.storage_performance" class="el-resourceId">
        ...
        <el-form-item label="数据盘存储设备" label-width="160px">
          <el-select v-model="hostListFrom.storage_capacity" class="el-resourceId">
      ...
      <div class="text-right">
        ...
        <button type="button" class="btn btn-sm btn-default firstbtn" :class="{ 'btn-choice': thirdFlag }"
          :disabled="!thirdFlag" @click="logError('ok')">
          取消
        </button>
        <button type="button" class="btn btn-sm btn-default" :class="{ 'btn-choice': thirdFlag }" :disabled="!thirdFlag"
          @click="logError('ok')">
          应用
        </button>
      </div>
